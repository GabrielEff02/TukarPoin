  ...
  static Future<Map<String, dynamic>> getInitData(String compan_code) async {
    ...
    try {
      final response = await http.get(Uri.parse(
          "$db/api/poin/checkouts-data?username=$username&compan_code=$compan_code"));
...
class LandingScreenData {
  static Future<Map<String, dynamic>> getCategoryData() async {
    try {
      var response = await http.get(Uri.parse("$db/api/poin/landing-data"));
  ...
  static Future<List<Map<String, dynamic>>> getCarouselData() async {
    try {
      var response = await http.get(Uri.parse("$db/api/poin/carousel-data"));
...
class Splash {
  static Future<List<String>> getSplashData() async {
    try {
      var response = await http.get(Uri.parse("$db/api/poin/splash-data"));
  ...
  static Future<Map<String, dynamic>> getNotification() async {
    try {
      ...
      var response = await http
          .get(Uri.parse("$db/api/poin/notifications?username=" + username));
      if (response.statusCode == 200) {
